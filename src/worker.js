...
async function askAi(request, env) {
  ...
  if (env.DEEPSEEK_API_KEY) {
    const response = await fetch(`${(env.DEEPSEEK_API_BASE || "https://api.deepseek.com").replace(/\/$/, "")}/chat/completions`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) throw new HttpError(502, "DeepSeek 服务暂时不可用");
  ...
  const result = await response.json();
  if (!response.ok) throw new HttpError(502, "AI 服务暂时不可用");
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      return json({ error: "服务器处理请求时发生错误" }, 500);
    }
  },
};
